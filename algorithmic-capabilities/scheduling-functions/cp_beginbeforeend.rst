.. aimms:function:: cp::BeginBeforeEnd(firstActivity, secondActivity, delay)
...
The function ``cp::BeginBeforeEnd(a,b,d)`` returns 1 if one of the
...
``cp::BeginBeforeEnd(a,b,d)`` is equivalent to

.. math:: \begin{array}{ll} a\texttt{.Present=0} & \vee \\ b\texttt{.Present=0} & \vee \\ a\texttt{.Begin} + d \leq b\texttt{.End} & \end{array}
